...
const mocks = vi.hoisted(() => ({ read: vi.fn(), setLocked: vi.fn(), unlink: vi.fn(), link: vi.fn(), lookup: vi.fn(), updateItems: vi.fn(), notify: vi.fn(), localItems: vi.fn(), transformPoint: vi.fn(), door: { id: "door", type: "IMAGE", metadata: {} } }));
...
vi.mock("./metadata", () => ({ readDoorJamMetadata: mocks.read, setDoorLocked: mocks.setLocked, removeDoorLink: mocks.unlink }));
vi.mock("./linking", () => ({ linkNearbyDoorOrCreate: mocks.link }));
vi.mock("./providers", () => ({ lookupDynamicFogDoor: mocks.lookup, INTEGRATION_NAMES: { dynamicFog: "Dynamic Fog" } }));
...
describe("door status overlay actions", () => {
  beforeEach(() => {
    ...
    mocks.lookup.mockReturnValue({ ok: false, reason: "missing-item" });
  });
  ...
  it("unlinks a linked door from its link glyph", async () => {
    ...
    expect(mocks.unlink).toHaveBeenCalledWith(door, metadata, "dynamicFog");
  });
